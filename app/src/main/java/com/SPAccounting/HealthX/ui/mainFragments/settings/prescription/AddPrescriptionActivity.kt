package com.SPAccounting.HealthX.ui.mainFragments.settings.prescription
...
import com.SPAccounting.HealthX.databinding.ActivityAddPrescriptionBinding
...
class AddPrescriptionActivity : AppCompatActivity() {
                    ...
                    .withListener(object : MultiplePermissionsListener {
                        override fun onPermissionsChecked(report: com.karumi.dexter.MultiplePermissionsReport) {
                            if (report.areAllPermissionsGranted()) {
                                ...
                                Toast.makeText(applicationContext, "Permissions are required to access files", Toast.LENGTH_SHORT).show()
                            }
                        }
    ...
    private fun process_upload(fileUri: Uri?) {
        ...
        if (userid.isNullOrEmpty()) {
            Toast.makeText(baseContext, "User ID not found. Please login again.", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        if (fileUri == null) {
            ...
            reference.putFile(fileUri).addOnSuccessListener {
                reference.downloadUrl.addOnSuccessListener { uri: Uri ->
                        // Update User object with new prescription URI
                        user.Prescription = uri.toString()
                        sharedPreference.saveUserToSharedPrefs(user)
